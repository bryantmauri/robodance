...
public class Competition {
	...
	//Method to select a list of robots
		...
		/*public List<String> totalTeamExperience(List<WebElement> teamNumber) {
			...
			List<WebElement> a = teamNumber;	      
			List<String> stringList =  a.stream().map(s -> s.findElement(RobotExperience).getText().trim()
												.split("Experience level: ")[1]).collect(Collectors.toList());
			
		    int integerList = stringList.stream().map(Integer::parseInt).collect(Collectors.summingInt(Integer::intValue));
	...
	//Method to start an automated dance (for going to leaderboards or simple tests) 
