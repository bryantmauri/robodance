...
public class Leaderboards {
	...
	//Method to return list of webelements for final losers
		...
		//Brings the left side of the robots (winner side) so we can compare if its the same as "final winners" panel
		@SuppressWarnings("null")
		public boolean compareWinnerNames() {
			List <String> winnersList = driver.findElements(winnersNames)
											.stream().map(s->s.getText()).collect(Collectors.toList());
			List <String> finalWinnersList = driver.findElements(finalWinnersNames)
											.stream().map(s->s.getText()).collect(Collectors.toList());
