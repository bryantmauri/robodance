  ...
  public void CheckTheLeaderboardButtonTest() throws InterruptedException {
	  System.setProperty("webdriver.chrome.driver", "C:\\Users\\bryan\\eclipse\\chromedriver.exe");
		...
		for(int i=0;i<RobotTeam1.size();i++) {
			...
			match.startDance();
			if(i==4) {
				a.assertTrue(match.goToLeaderBoardsIsDisplayed(),"Leaderboard Should be available after damce: "+ RobotTeam1.size());
			}else {
			a.assertFalse(match.goToLeaderBoardsIsDisplayed(),"Leaderboard Should not be available yet on dance: "+ i);
			}
		}		
		a.assertAll();
		driver.quit();	
