...
public class Team {
  ...
  public void onlyOneSelectedForBattlePerTeam() throws InterruptedException {
	  ...
	  System.setProperty("webdriver.chrome.driver", "C:\\Users\\bryan\\eclipse\\chromedriver.exe");
		...
		a.assertTrue(battleResult.trim().equals("Select for battle") || battleResult2.trim().equals("Select for battle") , 
		...
		System.out.println(driver.findElement(By.xpath("//app-robot-card[2]//div[1]//div[1]//p[2]")).getText().split("Experience level: ")[1]);
  ...
  public void ExperienceTest() {
	  System.setProperty("webdriver.chrome.driver", "C:\\Users\\bryan\\eclipse\\chromedriver.exe");
		...
		int sum1=0;
		int sum2=0;
		//I was trying to use streams here but i got stuck on the first value being repeated the list.size amount of times
		for(int i = 1; i <=RobotTeam1.size(); i++) {
			String s = driver.findElement(By.xpath("//app-team[1]/app-robot-card["+i+"]//div[@class='robot-details']/p[2]"))
		...
		for(int i = 1; i <=RobotTeam2.size(); i++) {
			String s = driver.findElement(By.xpath("//app-team[2]/app-robot-card["+i+"]//div[@class='robot-details']/p[2]"))
					.getText().split("Experience level: ")[1];
			int num = Integer.parseInt(s); 
			sum2=sum2+num;
		}
